...
function parseArgs() {
	...
	for (var c=2;c<process.argv.length;c++) {
		var arg = process.argv[c];
		if (re.test(arg)) {
			if (/^--version$/.test(arg) && /^(\d+.\d+.\d+)$/.test(process.argv[c+1])) {
				// remove the value if we're specifying a version value
				c++;
			}
			continue;
		}
...
function updateCheck(opts, callback) {
	// we are specifying a version, skip
	if (opts.version) { return callback(); }
