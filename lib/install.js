...
/**
 * find all native compiled modules
 */
...
function banner (callback) {
	...
	console.log(chalk.blue.underline.bold('Before you can continue, the latest Appcelerator software update needs to be downloaded.'));
	console.log();
...
/**
 * invoke the postinstall script
 */
function postInstall(installBin, callback) {
	var spawn = require('child_process').spawn,
		dir = path.join(path.dirname(installBin),'..'),
		fn = path.join(dir,'postinstall.js'),
		child = spawn(process.execPath, [fn], {cwd: dir, stdio:'inherit'});
 ...
 * run the install
 */
function install (installDir, opts) {
	...
	banner(function(err,result){
		...
		// download the binary
		download.start(url,wantVersion,function(err,filename,version) {
			...
			extract(filename, installDir, function(err,filename,dir){
				...
				compileNativeModules(dir, function(err){
					...
					// run the post install scripe
					postInstall(installBin, function(err) {
						if (err) { util.fail(err); }
