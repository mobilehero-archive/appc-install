			...
			return exec("xcode-select -p", function(err,stdout) {
				...
				if (exitCode===2) {
					...
					exec("gcc", function (err,stdout){
						return callback(new Error("Appcelerator tools require Xcode command line developers tools to be installed before continuing.  Choose an option in the dialog to download the command line developer tools."));
					});
				}
				else {
					callback();
...
function install (installDir, opts, cb) {
	...
	banner(function(err,result){
		...
		if (bin && !opts.force) {
			console.log('Version '+chalk.green(wantVersion)+' already installed.');
		...
		download.start(!!opts.force,url,wantVersion,function(err,filename,version,installBin) {
			...
			if (installBin) {
				console.log('Version '+chalk.green(version)+' already installed.');
