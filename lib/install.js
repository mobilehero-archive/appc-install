...
function preflight(opts, callback) {
	...
	if (!isWindows && (process.env.USER==='root' || process.getuid()===0)) {
		if (process.env.SUDO_USER) {
			return callback(new Error("Ooops! You cannot run using sudo. Please re-run using the "+process.env.SUDO_USER+" account."));
		}
		return callback(new Error("Ooops! You cannot run as root. Please re-run using a user account."));
	}
	// don't allow running as sudo from another user account.
	else if (!isWindows && (process.env.USERNAME==='root' && process.env.SUDO_USER)) {
		return callback(new Error("Ooops! You cannot run using sudo as another user. Please re-run using the "+process.env.SUDO_USER+" account."));
	}
	...
	if (!fs.existsSync(homedir)) {
		...
		return callback(new Error("Ooops! Your home directory ("+homedir+") cannot be found. Please make sure that the environment variable "+chalk.yellow('$'+envname)+" is set correctly to the correct directory and that it is writable."));
	}
			...
			return exec("xcode-select -p", function(err,stdout) {
				...
				if (exitCode===2) {
					...
					exec("gcc", function (err,stdout){
						return callback(new Error("Xcode command line developers tools are required.  Choose an option in the dialog to download the command line developer tools. Once you have completed the installation, please re-run this command."));
					});
				}
				else {
					callback();
	...
	gzip.extract(filename,dir, function(err) {
		...
		if (fs.existsSync(pkg)) {
			...
			if (attempts < 3) {
				...
				callback(new Error("Extraction failed. Please re-run again."));
			}
		}
	});
}
		...
		download.start(!!opts.force,url,wantVersion,function(err,filename,version,installBin) {
			...
			extract(filename, installDir, function(err,filename,dir){
				...
				compileNativeModules(dir, function(err){
					...
					if (!opts.quiet) { util.infoMessage(chalk.green.bold('Installed!!'+ (!opts.use&&!opts.setup?' Launching ... 🚀':''))); }
