...
function download(wantVersion, tmpfile, stream, location, callback, nobanner) {
		...
		if (res.statusCode===301 || res.statusCode===302) {
			...
			res.on('end', function () {
				...
				if (checkshasum!==shasum) {
					return callback(new Error("Invalid file download checksum. This could be a result of the file being modified in transit."));
				}
...
exports.start = function(location, wantVersion, callback) {
	...
	download(wantVersion, tmpfile, stream, location, callback);
};
